...
func TestAdmissionControlBoundsHostProvisioning(t *testing.T) {
	manager := NewFirecrackerManager(t.TempDir(), "./assets", "/bin/true", newTestDiskStore(t), 8, 2, 0, 0)
...
func TestAdmissionControlHasMinimumCapacity(t *testing.T) {
	manager := NewFirecrackerManager(t.TempDir(), "./assets", "/bin/true", newTestDiskStore(t), 8, 0, 0, 0)
